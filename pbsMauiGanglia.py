    ...
    # darn
                ...
                for i in rr:
                    if i % step == 0:
                        ...
                        jj.append( '%d_%d' % (k, i) )
                job_map[k] = jj
            elif j['array_task_id'] != None and j['array_job_id'] != None:  # running array tasks
...
#[(79619, ['gpu:2']), (79615, ['gpu:2'])]
            ...
            jobName = j['name'].replace('&', '_')  # an '&' in the job name will kill the xml stream...

            if j['job_state'] not in ( 'RUNNING', 'SUSPENDED' ):  # bale here and return minimal info...
                comment = ''
                if k in job_map.keys():
                    for i in job_map[k]:
                       self.queued.append( ( j['num_nodes'], j['num_cpus'], numGpus, j['job_state'], j['_username'], i, jobName, wallLimit, comment ) )
                continue
            ...
            if j['job_state'] == 'RUNNING':
               ...
               js='S'
            line = [ str(k), jobName, js, 'nodes %d' % j['num_nodes'], 'cores %d' % j['num_cpus'] ]
            if numGpus:
               line.append( 'Gpus %d' % numGpus )
            line.extend( ['mem req %dG' % (j['_ram']/1024), 'Wall time ' + wallTime_str, 'Remaining ' + intToTime(timeToGo) ] )
            cpuTime = 0 # @@@
            vmem = j['_ram'] # @@@
            eff = 0 # @@@
            info = { 'username':j['_username'], 'comment': '', 'wallLimit': wallLimit,
                     'numNodes':j['num_nodes'], 'mem':j['_ram'], 'numGpus':numGpus,
                     'cpuTime':cpuTime, 'jobName':jobName, 'vmem':vmem, 'state':js,
            ...
            if k in job_map.keys():
                for i in job_map[k]:
                    line[0] = i
                    info['jobId'] = i
                    self.jobs.append( ( j['_username'], nodes, gpus, line, self.tagId, timeToGo, i, jobName, info ) )
    ...
    def slurmRes( self ):
        ...
        for r, v in res_dict.items():
            if  v['start_time'] < time.time() < v['end_time']:   # active now
                if 'node_list' in v.keys() and v['node_list'] != None and len(v['node_list']) > 0:  # res might be license only. no nodes.
