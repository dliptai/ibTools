...
def parseToStats(r, lp):
    ...
    if lpData == lp:
        ...
        sys.stderr.write(
            sys.argv[0]
            + ": Error: got too much switch lid/port data back: asked for %d, got %d. scary. skipping it all.\n"
            % (len(lp), len(lpData))
        )
        return {}
    else:
        ...
        sys.stderr.write(
            sys.argv[0]
            + ": Error: expected %d responses, got %d." % (len(lp), len(lpData))
            + " Lid/Ports missing: "
            + str(compactPairs(errs))
            + "\n"
        )
        ...
        if len(errs):
            sys.stderr.write(
                sys.argv[0]
                + ": Error: too many Lid/Ports returned in data. skipping it all\n"
            )
            return {}
    ...
    for i in r:
        if i[:6] == "# Port":
            ...
            if len(d) != 4:
                sys.stderr.write(
                    sys.argv[0]
                    + ": Error: skipping timestamp: did not find 4 ib stats\n"
                )
                continue
            s[key] = (t, d)
        else:
...
def topologyChanged(cmd, lp):
    ...
    sys.stderr.write(
        sys.argv[0]
        + ": Info: found changed ibnetdiscover file. waiting 60s and reading again\n"
    )
    time.sleep(60)
